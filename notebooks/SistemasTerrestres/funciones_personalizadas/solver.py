# -*- coding: utf-8 -*-
"""
Created on Mon Oct 14 11:16:45 2019

@author: Dr. Victor Leonardo Teja
"""
import numpy as np
...
#@jit(nopython=True)
def algoritmo_thomas_2D(NX, NY, AP, AE, AW, AN, AS, B, FHI, iteramax, eps):
	Px = np.zeros(NX)
	Qx = np.zeros(NX)
	Py = np.zeros(NY)
	Qy = np.zeros(NY)
...
#----------------------- Calculo del residual --------------------------#
		...
		for i in range(1,NX-1):			
			for j in range(1,NY-1):
				...
				maxresold = maxres
#		maxres = max(Residual)
		#print ("i=",iteracion, "residual", maxres)
		iteracion+=1
