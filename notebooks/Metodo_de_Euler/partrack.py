...
def solveFlujo(Nt, ht, rango, w, dTray=True):
    ...
    # Definimos un Plotter para graficar.
    ...
    if dTray:
        for j in range(Np):
            vis.scatter(1, [t[j,0,0], t[j,-1,0]], [t[j,0,1], t[j,-1,1]],  ec='k', s=50, alpha=0.75, zorder=5)
            vis.plot(1, t[j,:,0], t[j,:,1], lw=2.0)
        vis.plot(1, t[:,:,0], t[:,:,1], ls = '--', color ='k', lw=.5)
...
#        plotFlujo(xg, yg, u, v, 'quiver', '')
        ...
        vis.streamplot(1, xg, yg, u(xg, yg), v(xg,yg), color='gray')
        vis.plot_frame(1, xg, yg)
